...
export function useNotifications() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.4) { // 40% 확률로 알림 생성
  ...
  const addRandomNotification = useCallback(() => {
    ...
    const randomLocation = randomType.locations.length > 0 ? 
      randomType.locations[Math.floor(Math.random() * randomType.locations.length)] : undefined;
    ...
    const newNotification: Notification = {
      ...
      timestamp: new Date(),
      read: false,
      userName: ['여행러김씨', '파리가이드', '도쿄포토그래퍼', '바르셀로나로컬'][Math.floor(Math.random() * 4)],
