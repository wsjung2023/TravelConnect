...
## 사이클론 통합 테스트 방법론 (Cyclone Integration Testing Methodology)

본 문서는 사이클론(Cyclone) 방법론을 적용하여 5개의 반복 루프를 통해 통합 테스트를 수행합니다.

### 루프 1: Scope Alignment (범위 조정)
**목표**: 요구사항과 페르소나 매핑 검증
- 각 테스트 시나리오가 실제 사용자 요구사항에 부합하는지 확인
- 페르소나별 기능 커버리지 분석

**진입 조건**: 요구사항 문서 및 UAT 피드백 준비 완료
**종료 조건**: 페르소나별 기능 매핑 완료, 테스트 시나리오 목록 확정
**산출물**: 테스트 시나리오 매트릭스, 페르소나-기능 매핑 테이블

### 루프 2: Systems Linkage (시스템 연결)
**목표**: 서비스/API 의존성 검증
- 프론트엔드 ↔ 백엔드 API 통신 검증
- 데이터베이스 CRUD 연산 검증
- 외부 서비스 통합 (Google Maps, PortOne 등)
- WebSocket 실시간 통신 검증

**진입 조건**: API 명세 및 스키마 확정
**종료 조건**: 모든 엔드포인트 통신 검증 완료, DB 마이그레이션 성공
**산출물**: API 테스트 리포트, 시스템 연결 다이어그램

### 루프 3: Scenario Storm (시나리오 폭풍)
**목표**: 기능 간 교차 여정 테스트
- 단일 기능이 아닌 사용자 여정 전체 테스트
- 엣지 케이스 및 오류 시나리오
- 동시성 및 경쟁 조건 테스트

**진입 조건**: 시스템 연결 검증 완료
**종료 조건**: 전체 사용자 여정 테스트 완료, 크리티컬 버그 0건
**산출물**: E2E 테스트 결과, 버그 리포트

### 루프 4: Signal Capture (신호 포착)
**목표**: 검증, 텔레메트리, 로컬라이제이션 확인

- 모든 API 응답 검증 (status code, body schema)
- 6개 언어 번역 정확성 검증
- 사용자 피드백 신호 (토스트, 알림) 확인

**진입 조건**: 기능 테스트 완료
**종료 조건**: 6개 언어 번역 검증 완료, UI 피드백 신호 정상 동작 확인
**산출물**: 번역 검증 리포트, 토스트/알림 테스트 결과

### 루프 5: Stability Sprint (안정성 스프린트)
**목표**: 회귀 테스트 및 장애 대응
- 기존 기능 회귀 테스트
- 네트워크 장애 시뮬레이션
- 데이터베이스 롤백 시나리오

**진입 조건**: 신호 포착 완료, 모든 기능 테스트 통과
**종료 조건**: 회귀 테스트 통과, 장애 대응 시나리오 검증 완료
**산출물**: 회귀 테스트 리포트, 장애 대응 매뉴얼
...
## Phase 16: 호스트 심사 플로우 (Host Application Review Flow)

### 개요
호스트 신청 후 관리자 심사를 거쳐 승인/거절되는 플로우 테스트

### 신규 API 엔드포인트
- `POST /api/user/apply-host` - 호스트 신청 (hostStatus: 'pending')
- `GET /api/user/host-status` - 신청 상태 조회
- `GET /api/admin/host-applications` - 대기 중 신청 목록
- `PATCH /api/admin/host-applications/:userId/approve` - 승인
- `PATCH /api/admin/host-applications/:userId/reject` - 거절

### 신규 스키마 필드
- `users.hostStatus`: varchar(20) - null | 'pending' | 'approved' | 'rejected'

### 테스트 시나리오

#### TC-P16-1: 호스트 신청 (Min-ji)
**사전 조건:**
- 일반 사용자로 로그인
- hostStatus: null, isHost: false

**테스트 절차:**
1. Profile 페이지에서 "호스트 되기" 버튼 클릭
2. 신청 확인 모달에서 "신청하기" 클릭
3. 프로필 페이지에서 상태 배지 확인
...
**테스트 절차:**
1. `/api/admin/host-applications` 조회
2. 특정 사용자 승인: `PATCH /api/admin/host-applications/{userId}/approve`
3. 해당 사용자 프로필 상태 확인
...
**테스트 절차:**
1. `PATCH /api/admin/host-applications/{userId}/reject` 호출
2. 해당 사용자 프로필 상태 확인
...
**예상 결과:**
- hostStatus: 'pending'으로 변경
- 심사 대기 상태로 전환

#### TC-P16-5: 호스트 상태 API 폴링 테스트
**테스트 절차:**
1. `/api/user/host-status` API 호출 (pending 상태)
2. 관리자 승인 후 다시 호출
3. 거절 상태에서 호출

**예상 결과:**
- pending 상태: `{ hostStatus: 'pending', isHost: false }`
- approved 상태: `{ hostStatus: 'approved', isHost: true }`
- rejected 상태: `{ hostStatus: 'rejected', isHost: false }`

#### TC-P16-6: 승인 후 호스트 전용 페이지 접근 검증
**사전 조건:**
- 관리자가 호스트 승인 완료 (isHost: true)

**테스트 절차:**
1. 로그아웃 후 승인된 사용자로 재로그인
2. 프로필 페이지 > "호스트 대시보드" 버튼 클릭
3. 호스트 대시보드 진입 확인
4. 체험 생성 버튼 클릭
...
### 16.1 체크리스트
- [ ] 호스트 신청 API 동작
- [ ] 상태 배지 UI 표시 (pending/rejected/approved)
- [ ] 관리자 승인 플로우
- [ ] 관리자 거절 플로우
- [ ] 재신청 플로우
- [ ] 호스트 대시보드 접근 권한
- [ ] 호스트 상태 API 폴링
- [ ] 승인 후 호스트 전용 페이지 접근 검증

---

## Phase 17: 북마크/저장된 포스트 (Bookmark/Saved Posts)

### 개요
사용자가 포스트를 저장하고 나중에 볼 수 있는 북마크 기능 테스트

### API 엔드포인트
- `POST /api/posts/:id/save` - 포스트 저장
- `DELETE /api/posts/:id/save` - 저장 취소
- `GET /api/user/saved-posts` - 저장된 포스트 목록

### 테스트 시나리오

#### TC-P17-1: 포스트 저장 (Yuki)
**테스트 절차:**
1. 피드에서 포스트 북마크 아이콘 클릭
2. 프로필 > 저장됨 탭 확인
...
#### TC-P17-3: 저장 API 실패 시나리오
**사전 조건:**
- 네트워크 불안정 또는 서버 오류 상황 시뮬레이션

**테스트 절차:**
1. 포스트 저장 버튼 클릭 (API 실패 시뮬레이션)
2. UI 상태 및 에러 처리 확인

**예상 결과:**
- 낙관적 업데이트 롤백 (아이콘 원상복귀)
- 에러 토스트 메시지 표시
- 쿼리 캐시 이전 상태로 복원
- 재시도 가능 상태 유지
...
## Phase 18: 가이드 프로필 개선 (Guide Profile Improvements)

### 개요
가이드 프로필 페이지의 뒤로가기 버튼 및 메시지 기능 테스트

### 신규 기능
- 뒤로가기 버튼 (ArrowLeft 아이콘)
- 메시지 보내기 버튼 (DM 생성 및 채팅 이동)

### 테스트 시나리오

#### TC-P18-1: 가이드 프로필 접근 및 뒤로가기 (Yuki)
**테스트 절차:**
1. 마켓플레이스에서 가이드 클릭
2. 가이드 프로필 페이지 표시 확인
3. 뒤로가기 버튼 클릭
...
**예상 결과:**
- 가이드 정보 표시 (이름, 소개, 체험 목록)
- 뒤로가기 시 이전 페이지로 이동

#### TC-P18-2: 가이드에게 메시지 보내기 (로그인 상태)
**사전 조건:**
- 일반 사용자로 로그인
...
**예상 결과:**
- DM 대화 생성 또는 기존 대화 찾기
- `/chat?conversationId={id}` 로 이동
- 채팅 UI에서 가이드와 대화 가능

#### TC-P18-3: 가이드에게 메시지 보내기 (비로그인 상태)
**사전 조건:**
- 로그아웃 상태
...
## Phase 19: 로컬라이제이션 회귀 테스트 (Localization Regression Testing)

### 개요
6개 언어(en, ko, ja, zh, fr, es) 번역 파일의 유효성 및 정확성 검증

### 주요 검증 항목
1. JSON 파싱 유효성
2. 필수 키 존재 여부
3. 플레이스홀더 일관성 ({{variable}})
4. 특수 문자 이스케이프
...
**테스트 절차:**
1. 각 언어 ui.json 파일 JSON.parse() 실행

**예상 결과:**
- 6개 파일 모두 파싱 성공
- 구문 오류 없음

#### TC-P19-2: 필수 키 존재 (호스트 상태 배지)
**검증 키:**
- `profile.hostPending`
- `profile.hostRejected`
...
#### TC-P19-3: 플레이스홀더 일관성
**예시 검증:**
- `host.totalReviews`: `{{count}}` 포함 여부
- `chat.sentAt`: `{{time}}` 포함 여부

**예상 결과:**
- 모든 언어에서 동일한 플레이스홀더 사용

### 19.1 체크리스트
- [ ] en/ui.json 유효성
- [ ] ko/ui.json 유효성
- [ ] ja/ui.json 유효성
- [ ] zh/ui.json 유효성 (최근 수정됨)
- [ ] fr/ui.json 유효성
- [ ] es/ui.json 유효성
- [ ] 신규 키 6개 언어 존재 확인
...
## Phase 20: 호스트-여행자 거래 (Host-Traveler Transactions)

### 개요
호스트와 여행자 간의 다양한 서비스 거래 플로우 테스트. 온라인/오프라인 서비스, 계약 생성, 에스크로, 마일스톤 결제, 분쟁, 정산까지 전체 거래 생명주기를 검증합니다.

### 관련 API 엔드포인트
- `POST /api/contracts` - 계약 생성
- `GET /api/contracts/:id` - 계약 조회
- `PATCH /api/contracts/:id/status` - 계약 상태 변경
- `POST /api/contracts/:id/milestones` - 마일스톤 추가
- `POST /api/contracts/:id/milestones/:milestoneId/fund` - 마일스톤 펀딩
- `POST /api/contracts/:id/milestones/:milestoneId/release` - 마일스톤 해제
- `POST /api/billing/escrow/deposit` - 에스크로 입금
- `POST /api/billing/escrow/release` - 에스크로 해제
- `POST /api/contracts/:id/disputes` - 분쟁 제기
- `GET /api/host/settlements` - 정산 내역 조회

### 서비스 유형
| 카테고리 | 온라인 서비스 | 오프라인 서비스 |
|----------|--------------|----------------|
| 정보 제공 | 예약 대행, 현지 정보 | - |
| 가이딩 | 온라인 투어, 화상 가이드 | 현장 가이드, 동행 투어 |
| 통역 | 실시간 채팅 통역 | 대면 통역 |
| 교통 | 예약 대행 | 픽업/드롭, 운전 |
| 숙박 | 예약 대행 | 홈스테이, 공유 숙소 |
| 어드벤처 | 가상 체험 | 액티비티 동행 |
| 쇼핑 | 대리 구매 | 쇼핑 동행, 흥정 도움 |

### 테스트 시나리오

#### TC-P20-1: 온라인 서비스 계약 - 예약 대행 (Min-ji → Marcus)
**페르소나:**
- 호스트: Min-ji (서울 로컬 가이드)
- 여행자: Marcus (독일 여행자)

**사전 조건:**
- Min-ji: 승인된 호스트 (isHost: true)
- Marcus: 일반 사용자, Trip Pass 활성화

**테스트 절차:**
1. Marcus가 Min-ji 체험 페이지 방문
2. "맞춤 서비스 요청" 버튼 클릭
3. 서비스 유형: "예약 대행" 선택
4. 요청 내용 작성 (DMZ 투어 예약 대행)
5. 예산: $50 USD 입력
6. DM으로 협상 시작
7. Min-ji가 맞춤 제안 작성 및 전송
8. Marcus가 제안 수락 → 계약 생성
9. 에스크로 결제 진행 (PortOne)
10. Min-ji가 서비스 완료 표시
11. Marcus가 서비스 확인 및 리뷰 작성
12. 에스크로 해제 → Min-ji 정산

**예상 결과:**
- 계약 상태: draft → pending → active → completed
- 에스크로: 결제 → 홀드 → 해제
- 정산: 다음 배치에서 Min-ji 계좌로 입금
- 알림: 각 단계별 양측에 알림 발송

#### TC-P20-2: 오프라인 서비스 계약 - 현장 가이드 (Min-ji → Sarah)
**페르소나:**
- 호스트: Min-ji
- 여행자: Sarah (미국 블로거)

**테스트 절차:**
1. Sarah가 Min-ji에게 DM으로 가이드 요청
2. Min-ji가 "명동 푸드 투어" 맞춤 제안 작성
3. 분할 결제 선택 (50% 선금, 50% 완료 후)
4. Sarah가 선금 결제
5. 투어 당일 진행
6. 투어 완료 후 잔금 결제
7. Sarah 리뷰 작성
...
**예상 결과:**
- 2단계 분할 결제 (milestones 2개)
- 각 마일스톤별 에스크로 처리
- 완료 후 전체 금액 정산

#### TC-P20-3: 분쟁 발생 시나리오 (Marcus → Min-ji)
**사전 조건:**
- 활성 계약 존재 (상태: active)
- 에스크로에 금액 홀드 중

**테스트 절차:**
1. Marcus가 서비스 불만족으로 분쟁 제기
2. 분쟁 유형: "서비스 품질" 선택
3. 분쟁 상세 내용 작성
4. Min-ji에게 분쟁 알림 발송
5. Min-ji 반박 의견 제출
6. 관리자 중재 진행
7. 부분 환불 결정 (30%)
8. 에스크로 분배: 70% 호스트, 30% 여행자

**예상 결과:**
- 분쟁 상태: open → under_review → resolved
- 부분 환불 처리
- 양측에 결과 알림

#### TC-P20-4: 계약 취소 및 전액 환불
**테스트 절차:**
1. 계약 생성 후 24시간 내 취소 요청
2. 취소 사유 입력
3. 전액 환불 처리
...
#### TC-P20-5: 에스크로 결제 실패 처리
**테스트 절차:**
1. 계약 생성 후 결제 진행
2. 결제 실패 시뮬레이션 (잔액 부족 또는 카드 오류)
3. 에러 처리 및 UI 상태 확인
...
#### TC-P20-6: 정산 배치 검증
**사전 조건:**
- 완료된 계약 다수 존재 (에스크로 해제됨)

**테스트 절차:**
1. 정산 배치 실행 (매일 02:00 KST)
2. `/api/host/settlements` API로 정산 내역 조회
3. PortOne Transfer API 호출 확인

**예상 결과:**
- 정산 상태: pending → completed
- 호스트 계좌로 입금 (수수료 10% 제외)
- 정산 내역 이메일 발송
- 정산 기록 DB 저장

### 20.1 체크리스트 (사이클론 루프별)
**Loop 1 - Scope Alignment:**
- [ ] 페르소나별 기능 매핑 완료 (호스트: Min-ji, 여행자: Marcus/Sarah)
- [ ] 서비스 유형 매트릭스 검증 (온라인/오프라인 7개 카테고리)
- [ ] 테스트 시나리오 목록 확정 (TC-P20-1 ~ TC-P20-6)

**Loop 2 - Systems Linkage:**
- [ ] 계약 생성 API (/api/contracts)
- [ ] 마일스톤 결제 API
- [ ] 에스크로 입금/해제 API
- [ ] PortOne 결제 연동
- [ ] WebSocket 실시간 알림

**Loop 3 - Scenario Storm:**
- [ ] 온라인 서비스 전체 플로우
- [ ] 오프라인 서비스 전체 플로우
- [ ] 분할 결제 (2단계, 3단계)
- [ ] 분쟁 제기 및 해결
...
**Loop 4 - Signal Capture:**
- [ ] 거래 단계별 알림 (6개 언어)
- [ ] PortOne 웹훅 수신 확인
- [ ] 결제 영수증 발송

**Loop 5 - Stability Sprint:**
- [ ] 결제 실패 시 롤백
- [ ] 네트워크 장애 복구
- [ ] 정산 배치 재시도
- [ ] 정산 배치 검증 (매일 02:00 KST)

---

## Phase 21: 여행자-여행자 만남 (Traveler-Traveler Meetups)

### 개요
여행자끼리 현지에서 만나는 기능 테스트. 미니밋(MiniMeet) 생성, 발견, 참여, DM 연결까지 전체 플로우를 검증합니다.

### 관련 API 엔드포인트
- `POST /api/social/mini-meets` - 미니밋 생성
- `GET /api/social/mini-meets/nearby` - 주변 미니밋 조회
- `POST /api/social/mini-meets/:id/rsvp` - 참여 신청
- `DELETE /api/social/mini-meets/:id/rsvp` - 참여 취소
- `GET /api/social/mini-meets/:id/participants` - 참여자 목록

### 테스트 시나리오

#### TC-P21-1: 미니밋 생성 및 발견 (Jack → Yuki)
**페르소나:**
- 주최자: Jack (호주 백패커)
- 참여자: Yuki (일본 여행자)

**사전 조건:**
- 두 사용자 모두 서울에 위치 (위도/경도)
- 로그인 상태
...
#### TC-P21-4: DM을 통한 오프라인 만남 연결
**테스트 절차:**
1. Yuki가 미니밋 채팅에서 Jack과 대화
2. 1:1 DM 전환 버튼 클릭
3. 개인 DM에서 상세 약속 조율
4. 위치 공유 기능 사용
...
#### TC-P21-5: 안전 가이드라인 및 신고 기능
**테스트 절차:**
1. 미니밋 참여 시 안전 가이드라인 모달 표시 확인
2. 부적절한 행동 신고 버튼 클릭
3. 신고 유형 선택 (스팸, 위협, 사기 등)
4. 신고 상세 내용 작성 및 제출

**예상 결과:**
- 안전 가이드라인 모달 표시 (6개 언어)
- 신고 접수 확인 토스트 표시
- 관리자에게 신고 알림 전송
- 신고 내역 DB 저장

#### TC-P21-6: 노쇼 처리 및 신뢰도 영향
**사전 조건:**
- 미니밋 참여 확정된 사용자
...
### 21.1 체크리스트 (사이클론 루프별)
**Loop 1 - Scope Alignment:**
- [ ] 페르소나별 기능 매핑 완료 (주최자: Jack, 참여자: Yuki)
- [ ] 미니밋 사용 시나리오 정의
- [ ] 테스트 시나리오 목록 확정 (TC-P21-1 ~ TC-P21-6)
...
## Phase 22: 호스트 이벤트 참여 (Host Event Participation)

### 개요
호스트가 주최하는 이벤트(체험)에 게스트들이 참여하는 전체 플로우 테스트. 이벤트 생성, 슬롯 예약, 결제, 출석 확인, 리뷰, 정산까지 검증합니다.

### 관련 API 엔드포인트
- `POST /api/experience/events` - 이벤트 생성
- `GET /api/experience/events/:id` - 이벤트 상세
- `GET /api/experience/events/:id/slots` - 슬롯 조회
- `POST /api/experience/events/:id/book` - 예약
- `DELETE /api/experience/events/:id/bookings/:bookingId` - 예약 취소
- `POST /api/experience/events/:id/checkin` - 체크인
- `POST /api/experience/events/:id/reviews` - 리뷰 작성

### 테스트 시나리오

#### TC-P22-1: 이벤트 생성 및 슬롯 설정 (Min-ji)
**페르소나:**
- 호스트: Min-ji (서울 로컬 가이드)

**테스트 절차:**
1. 호스트 대시보드에서 "이벤트 만들기" 클릭
2. 이벤트 정보 입력:
   - 제목: "서울 야경 포토 투어 🌃"
   - 설명: 남산타워, 청계천 야경 촬영
   - 가격: $45 USD
...
**예상 결과:**
- 이벤트 상태: draft → published
- 슬롯별 잔여석 표시
- 마켓플레이스에 이벤트 노출

#### TC-P22-2: 이벤트 예약 및 결제 (Li Wei → Min-ji 이벤트)
**페르소나:**
- 게스트: Li Wei (중국 여행자)

**테스트 절차:**
1. Li Wei가 마켓플레이스에서 이벤트 발견
2. 이벤트 상세 페이지 확인
3. 토요일 슬롯 선택
4. 인원: 2명 선택
5. 결제 진행 ($90 USD)
...
#### TC-P22-4: 이벤트 당일 출석 확인 (Min-ji)
**테스트 절차:**
1. Min-ji가 호스트 앱에서 출석 체크 시작
2. Li Wei 도착 → QR코드 스캔 또는 수동 체크인
3. 전원 출석 확인
4. 이벤트 진행

**예상 결과:**
- 출석 상태: pending → checked_in
- 노쇼 자동 표시 (30분 후)
- 이벤트 시작 알림

#### TC-P22-5: 이벤트 완료 후 리뷰 및 정산
**테스트 절차:**
1. 이벤트 종료 → 상태: completed
2. Li Wei에게 리뷰 요청 알림
3. Li Wei 리뷰 작성 (별점 5점)
4. 에스크로 해제 → Min-ji 정산
...
#### TC-P22-7: 게스트 예약 취소 및 환불
**페르소나:**
- 게스트: Li Wei

**테스트 절차:**
1. Li Wei가 예약 상세에서 "취소" 버튼 클릭
2. 취소 정책 확인 (48시간 전: 전액 환불)
3. 취소 사유 입력 및 확인
...
#### TC-P22-8: 대기 목록 자동 승격
**사전 조건:**
- 정원이 차서 대기 목록에 등록된 사용자 존재

**테스트 절차:**
1. 기존 예약자가 취소
2. 대기 목록 첫 번째 사용자에게 승격 알림 발송
3. 24시간 내 결제 진행
4. 미결제 시 다음 대기자로 기회 이전

**예상 결과:**
- 대기자에게 승격 알림 (6개 언어)
- 24시간 결제 마감 카운트다운 표시
- 결제 완료 시 예약 확정
- 미결제 시 자동 만료 및 다음 대기자 승격

#### TC-P22-9: 이벤트 정산 배치 검증
**사전 조건:**
- 완료된 이벤트 다수 존재 (에스크로 해제됨)
...
### 22.1 체크리스트 (사이클론 루프별)
**Loop 1 - Scope Alignment:**
- [ ] 페르소나별 기능 매핑 완료 (호스트: Min-ji, 게스트: Li Wei/Ana)
- [ ] 이벤트 예약 플로우 정의
- [ ] 테스트 시나리오 목록 확정 (TC-P22-1 ~ TC-P22-9)
...
| Phase 16 | 호스트 심사 플로우 | ✅ 완료 |
| Phase 17 | 북마크/저장된 포스트 | ✅ 완료 |
| Phase 18 | 가이드 프로필 개선 | ✅ 완료 |
| Phase 19 | 로컬라이제이션 회귀 테스트 | ✅ 완료 |
| Phase 20 | 호스트-여행자 거래 | ✅ 완료 |
| Phase 21 | 여행자-여행자 만남 | ✅ 완료 |
| Phase 22 | 호스트 이벤트 참여 | ✅ 완료 |
