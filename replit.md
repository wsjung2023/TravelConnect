...
## Guardrails (AI Agent ê°•ì œ ê·œì¹™ â€” ë§¤ ì„¸ì…˜ ë°˜ë“œì‹œ ì¤€ìˆ˜)
- **íŒŒì¼ í¬ê¸°**: íŒŒì¼ì´ 250ì¤„ ì´ˆê³¼ ì‹œ ë¶„ë¦¬ ê²€í† , 400ì¤„ ì´ˆê³¼ ì‹œ ë°˜ë“œì‹œ ë¶„ë¦¬. `server/routes.ts`ëŠ” í˜„ì¬ ë ˆê±°ì‹œ ê±°ëŒ€ íŒŒì¼ì´ë¯€ë¡œ ìƒˆ ì—”ë“œí¬ì¸íŠ¸ëŠ” ì ˆëŒ€ ì¶”ê°€í•˜ì§€ ë§ê³  `server/routes/` í•˜ìœ„ ëª¨ë“ˆë¡œ ë§Œë“ ë‹¤.
- **ì‹ ê·œ íŒŒì¼**: ëª¨ë“  ìƒˆ íŒŒì¼ ìƒë‹¨ì— 1ì¤„ ì„¤ëª… ì£¼ì„ í•„ìˆ˜ (`// ì´ íŒŒì¼ì˜ ì—­í• `).
- **ê²€ì¦ í•„ìˆ˜**: ì½”ë“œ ë³€ê²½ í›„ ë°˜ë“œì‹œ ì„œë²„ ì‹¤í–‰ í™•ì¸. ì‹¤í–‰í•˜ì§€ ì•Šì€ ì½”ë“œë¥¼ "ì™„ë£Œ"ë¼ê³  ë³´ê³  ê¸ˆì§€.
- **ë²”ìœ„ ìµœì†Œí™”**: í—ˆìš©ëœ íŒŒì¼ ì™¸ ìˆ˜ì • ê¸ˆì§€. í¬ë§·íŒ…/ë¦¬íŒ©í† ë§ ë“± ìš”ì²­ ì™¸ ì‘ì—… ê¸ˆì§€.
- **ì¶”ì¸¡ ê¸ˆì§€**: ë¡œê·¸ í™•ì¸ ì—†ì´ ì›ì¸ ë‹¨ì • ê¸ˆì§€. ë¶ˆí™•ì‹¤í•˜ë©´ ìˆ˜ì •í•˜ì§€ ë§ê³  ì§ˆë¬¸í•œë‹¤.
- **ëŒ€ê·œëª¨ ë³€ê²½ ê¸ˆì§€**: ì‚¬ìš©ì í™•ì¸ ì—†ì´ íŒŒì¼ ëŒ€ëŸ‰ ìƒì„±/ì‚­ì œ ê¸ˆì§€. ê¸°ì¡´ êµ¬ì¡° ë¬´ì‹œí•œ ë¦¬íŒ©í† ë§ ê¸ˆì§€.
- **ë©”ëª¨ ìœ ì§€**: ì¤‘ìš”í•œ ê²°ì •/ë°œê²¬ì€ `docs/AI_MEMO.md`ì— ì¦‰ì‹œ ê¸°ë¡.
...
- **Currency System**: Base currency USD, pricing tiers in USD, with PortOne/KGì´ë‹ˆì‹œìŠ¤ V2 payment gateway integration. Frontend displays prices using `Intl.NumberFormat` with `en-US` locale and `USD` currency.
...
## UI/UX v3 ¾÷±×·¹ÀÌµå ÇöÈ² (2026-03-20)

### ¿Ï·áµÈ ÀÛ¾÷
- **Ticket 1**: AppShell ÅëÇÕ ? AppBottomNav 5ÅÇ, ¸¸³ª±â ÅÇ ¹ÎÆ® ±Û·Î¿ì¸µ
- **Ticket 2**: µğÀÚÀÎ ÅäÅ« ? `--app-bg:#0A0B10`, `--accent-gold:#E6C989`, `--accent-mint:#7CE7D6`, `--accent-coral:#FF8A70`
- **Ticket 3**: Map ÅÇ ? MapTopBar(°ñµå µµ½Ã¸í+°Ë»öÇÊ+º§), MapFilterChips, PeopleMarker(¹ÎÆ® ±Û·Î¿ì¸µ SVG), MapBottomSheet(3´Ü ½º³À, ±Û·¡½º¸ğÇÇÁò)
- **Ticket 4**: Explore ÅÇ ? ExploreTopBar, CategoryTabs, ExploreCard(ÀÌ¹ÌÁö 200px+±×¶óµ¥ÀÌ¼Ç), 2¿­ ±×¸®µå
- **Ticket 5**: Meet ÅÇ ? RadarView(·¹ÀÌ´õ ½ºÄµ ¾Ö´Ï¸ŞÀÌ¼Ç), MeetTopBar, MeetFilterRow, UserCard(80px ¾Æ¹ÙÅ¸+¹ÎÆ® ±Û·Î¿ì+ÄÚ·² CTA), MeetStatusBanner
- **Ticket 6**: Chat ÅÇ ? ChatTopBar, ChatListItem(°ñµå ¾Æ¹ÙÅ¸¸µ+ÄÚ·² ÀĞÁö¾ÊÀ½ ¹èÁö+¹ø¿ª ¾ÆÀÌÄÜ), ChatList(¼½¼ÇÇì´õ)
- **Ticket 7**: Me/Profile ÅÇ ? ProfileHeader(96px+¹ÎÆ® ±Û·Î¿ì+Ä¿¹öÆ÷Åä), ProfileStats, ProfileLanguages, ProfileActivities, ProfileSettingsSection, ¿ªÇÒ ¹èÁö, 3¿­ Æ÷Åä±×¸®µå
- **Ticket 8**: ÇÇµå Æ÷½ºÆ® »ó¼¼ ? È÷¾î·Î ÀÌ¹ÌÁö, ÀÛ¼ºÀÚ Çà, °¡ÀÌµå Á¦¾È Ä«µå(ºí·ç º¸´õ), ´ñ±Û ¼½¼Ç
- **Ticket 9**: Ã¼Çè »ó¼¼ ? È÷¾î·Î 260px, È£½ºÆ® Ä«µå(ShieldCheck ¹îÁö), ÀÎÆ÷Ä¨, È®Àå ¼³¸í, ¹Ì´Ï Ä¶¸°´õ, ¸®ºä, ½ºÆ¼Å° ¹ÙÅÒ¹Ù
- **Ticket 10**: ¼­ºñ½º ¿äÃ» ? ¿äÃ»¼­ Æû v3 ½ºÅ¸ÀÏ
- **Ticket 11**: P2P ¿¡½ºÅ©·Î °è¾à ? °è¾à È­¸é v3 ½ºÅ¸ÀÏ
- **Ticket 12**: ¼¼·±µğÇÇÆ¼ ¸ğ¸ÕÆ® ? ´ëÇü ¹ÎÆ® ¿ÀºñÅ» ¸µ, µÎ ¾Æ¹ÙÅ¸ ¸ÅÄª ½Ã°¢È­, ¹Ì´Ï¸Ê, CTA

### Àü¿ª ½ºÅ² (2026-03-20 Àû¿ë)
- ÀüÃ¼ ¹è°æ: ¼ø¼ö ºí·¢ `#0A0B10` (º¸¶ó °è¿­ Àü¸é Á¦°Å)
- ¸ğµç Ä«µå: `border: 1px solid rgba(200,168,78,0.2)` + `box-shadow: 0 0 12px rgba(200,168,78,0.08)`
- ÅÇ¹Ù: `backdrop-filter: blur(10px)`, È°¼ºÅÇ °ñµå
- ¹ÙÅÒ½ÃÆ®: `backdrop-filter: blur(16px)` + ±Û·¡½º¸ğÇÇÁò
- ¸ğµç CTA ¹öÆ°: `var(--accent-coral)` #FF8A70
- ¾Æ¿ô¶óÀÎ ¹öÆ°: °ñµå Å×µÎ¸®
- Áöµµ ¸¶Ä¿: ¿øÇü ¾Æ¹ÙÅ¸ + ¹ÎÆ® ³×¿Â ±Û·Î¿ì¸µ SVG

### i18n Seed ½ºÅ©¸³Æ® (Replit¿¡¼­ 1È¸ ½ÇÇà ÇÊ¿ä)
```
...
### ÁøÇà Áß / ÀÜ¿© ÀÛ¾÷
- [ ] ¼¼·±µğÇÇÆ¼ ÆäÀÌÁö ´ëÇü ¿ÀºñÅ» ¸µ ½Ã°¢È­ ÃÖÁ¾ ¿Ï¼º
- [ ] ÀüÃ¼ ¾Û ÆäÀÌÁö ¼øÈ¸ °ñµå/´ÙÅ© Å×¸¶ ÀÏ°ü¼º °ËÁõ
- [ ] Replit¿¡¼­ seed ½ºÅ©¸³Æ® ½ÇÇà (i18n Å° DB µî·Ï)
- [ ] ÄÁ¼Á¾ÆÆ® 10°³¿Í ½ÇÁ¦ È­¸é 1:1 ½ºÅ©¸°¼¦ ºñ±³ °ËÁõ
