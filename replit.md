...
### Translations API Returns HTML Instead of JSON (2025-12-22)

**Problem**: `/api/translations/interests` API 요청 시 JSON 대신 HTML(index.html)이 반환됨. OnboardingModal에서 interests 버튼이 표시되지 않음.

**Root Cause**: 
프로젝트에 두 개의 라우트 파일이 존재:
- `server/routes.ts` - **실제 사용되는 파일** (`registerRoutes` 함수)
- `server/routes/index.ts` - 모듈식 라우터용 (사용되지 않음)

`server/index.ts`에서 `import { registerRoutes } from './routes'`는 `server/routes.ts`를 가리킴.
Translations API를 `server/routes/index.ts`에 추가했지만, 이 파일은 실제로 사용되지 않아 API가 등록되지 않음.

**Failed Attempts**:
1. `server/routes/index.ts`에 API 추가 → 라우트가 등록되지 않음
2. 디버깅 로그 추가 → 로그가 출력되지 않음 (라우트 미등록)
3. Vite 미들웨어 순서 확인 → 문제 없음

**Solution**:
`server/routes.ts`에 translations API 추가:
```typescript
// server/routes.ts
...
**Key Takeaways**:
- ⚠️ **라우트 파일 위치 확인 필수**: `import` 경로가 어느 파일을 가리키는지 확인
- ⚠️ **로그가 출력되지 않으면 코드가 실행되지 않는 것**: 라우트가 등록되지 않았을 가능성
- ✅ Express에서 `/routes`는 `routes.ts` 또는 `routes/index.ts` 둘 다 가능 - 혼란 주의

**Cost**: 디버깅 시간 소요 - 두 파일 간의 혼동으로 인해 잘못된 파일에 코드 추가