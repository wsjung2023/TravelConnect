...
import { MapPin, Camera, Video, Heart, Cloud, Waves, Image as ImageIcon, X, Calendar, Clock, Hash, Upload, Film, Search } from "lucide-react";
...
export function JourneyCreateModal({ isOpen, onClose, prefilledLocation }: JourneyCreateModalProps) {
      ...
      if (savedDraft) {
        try {
          ...
          if (Date.now() - draftData.timestamp < 30 * 60 * 1000) {
            ...
            setSelectedDate(draftData.selectedDate || new Date().toISOString().split('T')[0]);
            setSelectedTime(draftData.selectedTime || new Date().toTimeString().slice(0, 5));
  ...
  const handleVideoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    videoFiles.forEach(video => {
      if (video.size > 100 * 1024 * 1024) { // 100MB 제한
        toast({
          title: "업로드 제한",
  ...
  const handleSubmit = async () => {
    ...
    if (!content.trim() && uploadedImages.length === 0 && uploadedVideos.length === 0) {
      toast({
        title: "콘텐츠 입력 필요",
    ...
    try {
      ...
      // 2. 게시물 생성
      const tagsArray = tags.split(' ').filter(tag => tag.trim().startsWith('#'));
      ...
      const response = await apiRequest("/api/posts", "POST", {
        ...
        longitude: selectedLocation?.lng ? selectedLocation.lng.toString() : null,
        title,
        day: day && day.trim() !== "" ? parseInt(day) : 1,
        shape: selectedShape,
        theme: selectedTheme,
      ...
      toast({
        title: "오류 발생",
        description: error instanceof Error ? error.message : "여정 기록 중 오류가 발생했습니다.",
        variant: "destructive",
      });
    } finally {
        ...
        <div className="space-y-4 p-4 pb-6">
          ...
          <div className="space-y-4 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
            ...
            <div className="text-xs text-blue-600 dark:text-blue-400">
              업로드된 이미지: {uploadedImages.length}개 | 동영상: {uploadedVideos.length}개
            </div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {uploadedImages.map((image, index) => {
                        ...
                        if (!image || !image.name || image.name === "") {
                          return (
                            <div key={index} className="relative bg-gray-100 rounded-lg p-4 text-center">
                              <div className="text-xs text-gray-500">이미지 #{index + 1}</div>
                              <button
          ...
          <div className="space-y-3 bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200">📅 여행 타임라인</h3>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="no-timeline" className="text-sm text-gray-700 dark:text-gray-300">
                      타임라인 없이 단독 포스팅
                    </label>
                  </div>
                      ...
                      {timelines.map((timeline: any) => (
                        <div key={timeline.id} className="flex items-center gap-2">
                          <input
                            ...
                            onChange={() => setSelectedTimelineId(timeline.id)}
                            className="w-4 h-4 text-purple-600"
                          />
                          <label htmlFor={`timeline-${timeline.id}`} className="text-sm text-gray-700 dark:text-gray-300 flex-1">
                            {timeline.title} 
                            {timeline.destination && (
                              <span className="text-xs text-gray-500 ml-1">- {timeline.destination}</span>
                            )}
                          </label>
                        </div>
                      ))}
                  ...
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      console.log("피드 작성 내용 임시저장하고 타임라인 페이지로 이동");
                      ...
                      const draftData = {
                        ...
                        tags,
                        selectedLocation,
                        uploadedImages: uploadedImages.map(file => ({ name: file.name, size: file.size })),
                        uploadedVideos: uploadedVideos.map(file => ({ name: file.name, size: file.size })),
                        timestamp: Date.now()
                      };
                      localStorage.setItem('feedDraft', JSON.stringify(draftData));
                      
                      onClose(); // 현재 모달 닫기
                      window.postMessage({ type: 'open-timeline-modal' }, '*'); // 기존 타임라인 모달 열기
                    }}
